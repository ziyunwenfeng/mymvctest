		...
		<table width="100%" border=1>
			...
			<tbody>
				<c:forEach items="${itemsCustomList }" var="itemsCustom">
					<tr>
						<td><input type="checkbox" name="itemsId"
							value="${itemsCustom.id}"></td>
						<td>${itemsCustom.id}</td>
						<td>${itemsCustom.name}</td>
						<td>${itemsCustom.price}</td>
						<td>${itemsCustom.picture}</td>
						<td><fmt:formatDate value="${itemsCustom.createtime}"
								pattern="yyyy-MM-dd HH:mm:ss" /></td>
						<td>${itemsCustom.detail}</td>
						<td><a
							href="${pageContext.request.contextPath}/items/edit_items?id=${itemsCustom.id}">修改</a></td>
						<td><a
							href="${pageContext.request.contextPath}/items/edit_items2?id=${itemsCustom.id}">修改2</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
